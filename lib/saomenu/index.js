...
function SaoMainMenuController($scope, bindKeys) {
  ...
  function reposition() {
    if(currentMenuIdx === 0) {
      // get top element
      var zero = $('.top .icon:first-child');
      // get target element
      var menuIdx = selectedIndexes[0];
      var target = $('.top .icon:nth-child('+(menuIdx+1)+')');
      // compute initial - final for the negative offset
      var offset = zero.offset().top - target.offset().top;
      // animate to that offset
      $('.top .icon').animate({ top: offset }, 150);
    } else {
      // TODO reposition sub menus
    }
  }
