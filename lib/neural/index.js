    ...
    function worldLoop() {
      ...
      munchers.forEach(function(m) {
        ...
        if(m.auto) {
          var closest_angle = Math.min(
            ...
            ((m.brain.input[2]/2 > m.brain.input[0])?Infinity:m.brain.input[3]),
            // if the third food is too far, then ignore it's angle
            ((m.brain.input[4]/2 > m.brain.input[2])?Infinity:m.brain.input[5])
          );
          // basic logic
      ...
      if(someDead) {
        ...
        munchers
          ...
          .forEach(function(m) { m.train(munchers); });
