    ...
    function worldLoop() {
      ...
      munchers.forEach(function(m) {
        ...
        foods.map(function(f) { return [
          utils.distance(m.x, m.y, f.x, f.y), // TODO normalize
          utils.wrap(-Math.PI, Math.atan2(f.y-m.y, f.x-m.x) - m.a, Math.PI), // TODO normalize?
        ]; }).sort(function(f1, f2) {
          return f1[0] - f2[0];
        ...
        if(m.auto) {
          // basic logic
          if(-Math.PI/6 < m.brain.input[1] && m.brain.input[1] < Math.PI/6) {
            // straight
            m.nextLeft = false;
            m.nextRight = false;
          } else if(-Math.PI/2 < m.brain.input[1] && m.brain.input[1] < 0) {
            // slight left
            m.nextLeft = true;
            m.nextRight = false;
          } else {
