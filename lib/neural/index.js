...
module.exports.controller('spinningnode.neural.app.controller', [
  ...
  function($scope, $timeout, bindKeys) {
    ...
    function worldLoop() {
      ...
      if(munchers[0].auto) {
        ...
        munchers
          ...
          .forEach(function(m) { m.train(munchers); m.trainer--; });
      }
    ...
    function statusLoop() {
      // if all but 1 munchers are near the edge, then shuffle
      var gutter = Muncher.radius * 4;
      var nearEdge = munchers.reduce(function(c, m) {
        if(m.x < gutter || m.y < gutter || m.x > WORLD_WIDTH - gutter || m.y > WORLD_HEIGHT - gutter)
          return c+1;
        return c;
      }, 0);
      if(nearEdge > munchers.length-2)
        $scope.shuffle();
...
Muncher.prototype.train = function(munchers) {
  this.trainer++;
  var memory = this.memory;
  munchers.forEach(function(m) {
    m.trainee++;
    for(var loop=TRAIN_LOOP_COUNT; loop>0; loop--)
      memory.forEach(function(r) { m.brain.train(r[1], r[0]); });
  });
};
