...
Skill.Detect_Power = function() { return new Skill('Detect Power', 10); };
Skill.Health_Regen = function() { return new Skill('Health Regen', 2); };
...
Skill.prototype.add = function(num) {
  ...
  while(this.curr >= this.level * this.step) {
    this.curr -= this.level * this.step;
    this.level++;
  }
};
