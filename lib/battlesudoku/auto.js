...
//  - you don't need to remember the states, just the index
...
//    -, -, 3, 3, 3, 3, -
//    -, -, -, 4, 4, 4, 4
//  - XXX bbuuttt... not all places are mutually exclusive, and the later locations might produce different results
//    - so only do it when the previous and current produce the same list?
//    - will this ever work since we placed a piece on the previous step?
//    - maybe it's beneficial to find ALL the places for a given length size and address them all at once, to force the iteration across the specified set
function recursiveSolve(available, lengths, counts, Promise, notify, prevL) {
  ...
  var l = lengths.pop();
  var possible = findAllLengthsForL(available, l, counts);
