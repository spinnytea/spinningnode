...
exports.solve = function solve(b, Promise, notify) {
  ...
  var lengths = _.sortBy(_.map(b.len.filter(function(l) { return !l.done; }), 'size'), function(l) { return +l; });
  var counts = {
...
function findMustPossibilities(a, m, lens) {
  var possible = [];
  var len = { d: m.d };

  // FIXME account for counts
  lens.forEach(function(l) {
    ...
    if(m.d) {
      for(;m.c + m.l <= len.c + len.l; len.c--) {
        if(canPlace(a, len.r, len.c, len.l, len.d))
...
function pickAMust(available, lengths) {
  ...
  return findAllMust(available).reduce(function(best, must) {
    ...
    if(must.l === 1) {
      ...
      next = findMustPossibilities(available, must, uniqueLengths);
      must.d = false;
      Array.prototype.push.apply(next, findMustPossibilities(available, must, uniqueLengths));
    } else {
      next = findMustPossibilities(available, must, uniqueLengths);
    }
...
function recursiveMust(available, lengths, counts, Promise, notify) {
  ...
  var possible = pickAMust(available, lengths);
  if(possible.length === 0) {
