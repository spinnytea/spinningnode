...
exports.solve = function solve(b) {
  ...
  var available = b.map(function(r) {
    return r.map(function() {
      return true;
    });
  });
  var lengths = _.map(b.len, 'size').sort();
  var counts = {
    row: _.map(b.row, 'total'),
    col: _.map(b.col, 'total'),
  };

  // find solution
  var result = exports.units.recursiveSolve(available, lengths, counts);

  // apply available to b
  result.forEach(function(row, r) {
    row.forEach(function(cell, c) {
      b[r][c].state = (cell?'empty':'fill');
    });
  });
};
...
function recursiveSolve(available, lengths, counts) {
  ...
  var l = lengths.pop();
  var possible = combine(
    _.flatten(_.filter(counts.row.map(function(count, r) {
      if(count >= l)
        return findRowLengths(available, r, l);
      return undefined;
    }))),
    _.flatten(_.filter(counts.col.map(function(count, c) {
      if(count >= l)
        return findColLengths(available, c, l);
      return undefined;
    })))
  );

  // TODO recurse single
  // - if there is only one path, then we don't need to clone all our data structures
  // XXX should recurse single be the "standard final case"
  // - even when there are multiples, we don't need to clone data on the last iteration
  return possible.reduce(function(ret, p) {
    if(!ret) {
      // update available
      var a = _.cloneDeep(available);
      doPlace(a, p.r, p.c, p.l, p.d);

      // update counts
      var totals = getCounts(a);
      var c = {
        row: counts.row.map(function(value, index) { return value-totals.row[index]; }),
        col: counts.col.map(function(value, index) { return value-totals.col[index]; }),
      };

      // copy lengths
      var l = _.cloneDeep(lengths);
