...
/**
 * TODO come up with a way to honor partial information (cells marked as 'empty', of 'fill' that isn't a length)
 * @param b a board
 */
...
function recursiveSolve(available, lengths, counts) {
  ...
  var possible = combine(
    _.flatten(_.filter(counts.row.map(function(count, r) {
      if(count >= l)
        return findRowLengths(available, r, l, counts.col);
      return undefined;
    }))),
    _.flatten(_.filter(counts.col.map(function(count, c) {
      if(count >= l)
        return findColLengths(available, c, l, counts.row);
      return undefined;
    })))
  );
  ...
  return possible.reduce(function(ret, p) {
    if(!ret) {
      ...
      // update counts
      var c = updateRecursiveCounts(counts, p);
...
function updateRecursiveCounts(counts, p) {
  var c = _.cloneDeep(counts);
  var i;
  if(p.d) {
    c.row[p.r] -= p.l;
    for(i=0; i<p.l; i++)
